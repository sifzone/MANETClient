...
public class DijkstraRoutingAlgorithm {
	...
	public DijkstraRoutingAlgorithm(String source, ConcurrentHashMap<String, List<AdjacentNode>> graph){
		...
		/**
		 * Lets distance of start vertex from start vertex = 0 
		 * Lets distance of all other vertices from start vertex = Infinity or Max Value
		 */		
		Node node;
		...
		/**
		 * Loop
		 * 	Visit the unvisted vertex with the smallest known distance from the start vertex
		...
		while(!unvisited.isEmpty()){
			...
			for(AdjacentNode currentNeighbor : neighborsCurrentNode){
				...
				if(unvisited.contains(newNode)						
						&& (newNode.getDistanceFromSource() >= currentNode.getDistanceFromSource() + currentNeighbor.getWeight()) ){
